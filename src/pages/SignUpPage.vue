...
const handleSignUp = async () => {
  if (!username.value || !email.value || !password.value) {
    message.warning("Nom d'utilisateur, email et mot de passe requis")
    return
  }
  ...
  try {
    ...
    message.error((err as Error).message || "Échec de l'inscription")
  } finally {
    loading.value = false
  }
}
...
<template>
  <NSpace id="space" vertical align="center" justify="center">
    <NForm id="form" @submit.prevent="handleSignUp">
      <h1>Inscription</h1>
      ...
      <p id="redirection">
        Déjà un compte ?
        <RouterLink :to="ROUTES.SIGNIN">Se connecter</RouterLink>
      </p>
    </NForm>
  </NSpace>
</template>
